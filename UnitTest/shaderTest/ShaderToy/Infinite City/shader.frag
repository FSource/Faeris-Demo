...
vec2 Rand( vec2 pos )
{
	return texture2D( u_texture0, (pos+.5)/256.0, -100.0 ).xz;
}
...
/*	float f = Voronoi(gl_FragCoord.xy/40.0).z;
	...
	vec3 rayDir = normalize( vec3( gl_FragCoord.xy-iResolution.xy*.5, iResolution.x * zoom ) );
	...
	vec3 reflection = texture2D( u_texture0, reflect(rayDir,norm).xz/8.0 ).rgb*3.0;
