...
vec3 shadeplanet(vec3 pos, vec3 k) { 
	...
	// add some noise
	float noi=max(0.,texture2D(u_texture0,n.yz*.5).x-.6);
	noi+=max(0.,texture2D(u_texture0,n.yz).x-.6);
	noi+=max(0.,texture2D(u_texture0,n.yz*2.).x-.6);
	col+=noi*(1.5-c)*.7;
...
void main(void)
{
	...
	if ((mod(iGlobalTime,20.)>15. && iMouse.z<1.)) {
		...
		//from.x+=1.; from.y+=.5;
		vec2 fixpos=(texture2D(u_texture0,vec2(from.z*.21325)).xy-.5)*vec2(FOLD*2.-.3,1.);
